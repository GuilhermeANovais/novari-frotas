...
// Removemos o import de Pagination
...
// Alterado: Constante para incremento em vez de página fixa
...
export function DepartmentDetails() {
  ...
  // Alterado: Estado para controlar quantidade visível (Load More)
  ...
  // Efeito para Resetar a contagem ao mudar filtros
  useEffect(() => {
  ...
  const handleDeleteVehicle = async (vehicle: Vehicle) => {
    if (!window.confirm(`Excluir veículo ${vehicle.licensePlate}?`)) return;
    try {
  ...
  // --- Lógica de Corte (Slice) para Load More ---
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 animate-fade-in pb-20">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
        <div className="flex items-center gap-2">
          <button onClick={() => navigate('/')} className="text-zinc-500 hover:text-zinc-900 transition-colors p-2 rounded-full hover:bg-zinc-100">
        ...
        {activeTab !== 'logs' && (
          <div className="flex gap-2">
            <Button onClick={() => activeTab === 'vehicles' ? handleOpenCreateVehicle() : handleOpenCreateDriver()}>
              <Plus className="w-5 h-5 mr-2" /> Adicionar {activeTab === 'vehicles' ? 'Veículo' : 'Motorista'}
            </Button>
          </div>
        )}
      </div>
      ...
      <div className="border-b border-zinc-200 mb-6">
        <nav className="-mb-px flex space-x-8">
           <button onClick={() => setActiveTab('vehicles')} className={`pb-4 px-1 border-b-2 font-medium text-sm flex items-center transition-colors ${activeTab === 'vehicles' ? 'border-zinc-900 text-zinc-900' : 'border-transparent text-zinc-500'}`}><Car className="w-5 h-5 mr-2" /> Veículos</button>
           <button onClick={() => setActiveTab('drivers')} className={`pb-4 px-1 border-b-2 font-medium text-sm flex items-center transition-colors ${activeTab === 'drivers' ? 'border-zinc-900 text-zinc-900' : 'border-transparent text-zinc-500'}`}><User className="w-5 h-5 mr-2" /> Motoristas</button>
           <button onClick={() => setActiveTab('logs')} className={`pb-4 px-1 border-b-2 font-medium text-sm flex items-center transition-colors ${activeTab === 'logs' ? 'border-zinc-900 text-zinc-900' : 'border-transparent text-zinc-500'}`}><History className="w-5 h-5 mr-2" /> Histórico</button>
        </nav>
      </div>
      ...
      {activeTab !== 'logs' && (
        <div className="bg-white p-4 rounded-lg shadow-sm border border-zinc-200 mb-6 grid grid-cols-1 md:grid-cols-3 gap-4">
          <div className="relative">
             ...
             {/* Atualizado placeholder conforme a imagem */}
             <Input 
          ...
          <div className="flex bg-zinc-100 p-1 rounded-lg">
             {[{ label: 'Todos', value: '' }, { label: 'Atenção', value: 'error' }, { label: 'Alerta', value: 'warning' }, { label: 'Ok', value: 'ok' }].map((opt) => (
               <button key={opt.value} onClick={() => setStatusFilter(opt.value)} className={`flex-1 text-sm font-medium rounded-md py-1 transition-all ${statusFilter === opt.value ? 'bg-white shadow text-zinc-900' : 'text-zinc-500'}`}>{opt.label}</button>
             ))}
          </div>
        </div>
      )}
      ...
      {activeTab === 'vehicles' && (
        <>
              ...
              return (
                <div key={vehicle.id} className="bg-white rounded-lg shadow-sm hover:shadow-md transition-all duration-200 overflow-hidden flex flex-col border border-zinc-200">
                  ...
                  <div className="p-4 flex-grow">
                    ...
                    <div className="space-y-2 text-sm">
                      <div className="flex justify-between border-b border-zinc-100 pb-1">
                        ...
                        <span className="font-medium text-zinc-800 truncate max-w-[150px]">{vehicle.driverName || 'N/D'}</span>
                      </div>
                      <div className="flex justify-between border-b border-zinc-100 pb-1">
                  ...
                  <div className="bg-zinc-50 p-3 border-t border-zinc-100 flex justify-between items-center">
                    <button onClick={() => handleDeleteVehicle(vehicle)} className="text-zinc-400 hover:text-white hover:bg-red-600 transition-colors p-2 rounded-md"><Trash2 className="w-4 h-4" /></button>
                    <div className="flex gap-2">
          ...
          {/* Botão Carregar Mais - Substituindo Paginação */}
                ...
                <div key={driver.id} className={`p-6 rounded-lg shadow-sm border hover:shadow-md transition-shadow ${statusBg} flex flex-col justify-between`}>
                  ...
                  <div className="flex items-center justify-between mt-4 pt-4 border-t border-zinc-200/50 text-sm">
                    <div>
                       ...
                       <span className={`font-bold ${status === 'error' ? 'text-red-600' : 'text-green-600'}`}>{formatDate(driver.licenseExpiration)}</span>
                    </div>
                    <div className="flex items-center gap-2">
